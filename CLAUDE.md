...
8. **Real node, 2026-08-06 (later) — the pre-0.16.0 sweep, all three repos.** Same node, same batch `c931c8a5…` (TTL 31.8 days), throwaway signers. Every gated test in the family run, nothing left skipped: **ontodag 2/2** (43 s — the CLI swarm backend and the scorched-earth feed rehydration, i.e. the morning's bootstrap fix still holds), **recordstore 21/21** (3:56 — `test_recordstore_bee` + `test_recordstore_feed` + `test_swarm_store` at the 0.20.0 tag's code, so the published release is live-validated; the feed suite dominates the wall clock, as in runs 5–6), and **ontodag-fs 1/1** (its `test_live_bee`, real BMT refs for file content, against this repo's 0.16.0 candidate — the same thing the publish workflow's downstream gate runs). Nothing new learned about the node; the point of the run is that the release candidate has no unexercised Swarm path. **Re-run an hour later, after the surface-parity wave** (which touched `Session._load` and the backends for `--as-of`): ontodag 2/2 again (42 s), and ontodag-fs's whole suite with the gate open — 283 passed, 0 skipped.
