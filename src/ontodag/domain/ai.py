"""The `ai` pack, version 3: 675 categories, adopted by merge.

GENERATED — do not edit by hand. Built by consensus in the sister repo
github.com/petfold/ontodag-core (packs/ai, commit fb54269) from WordNet 3.0
and Wikidata with hand rulings; every name is a plain word that no everyday
sense already owns (a pack never takes an everyday word). It presumes `core`,
which `pack ai` applies first; core's names appear below only as parents.
`BORROWED` are names that belong to a sibling pack — entered parentless so
this pack adopts on its own, filed the moment the sibling is adopted
(refinement by merge). ontodag-core's docs/UPPER.md §8 is the record.
"""

VERSION = 3

BORROWED = ('database',)

# (name, parents) — sorted by name.
PACK = (
    ('3d-reconstruction', ('computer-vision',)),
    ('a-star-search', ('best-first-search',)),
    ('abductive-logic-programming', ('logic-programming',)),
    ('abductive-reasoning', ('thinking',)),
    ('activation-function', ('concept',)),
    ('active-learning', ('machine-learning',)),
    ('actor-critic-algorithm', ('reinforcement-learning',)),
    ('actuator', ('device',)),
    ('adam-optimizer', ('stochastic-gradient-descent',)),
    ('adaptive-control', ('control-theory',)),
    ('adversarial-example', ('concept',)),
    ('adversarial-machine-learning', ('machine-learning',)),
    ('adversarial-training', ('adversarial-machine-learning',)),
    ('affective-computing', ('artificial-intelligence',)),
    ('agent-based-model', ('computer-simulation',)),
    ('agricultural-robot', ('automaton',)),
    ('ai-agent', ('intelligent-agent',)),
    ('ai-alignment', ('ai-safety',)),
    ('ai-model', ('concept',)),
    ('ai-safety', ('artificial-intelligence',)),
    ('algorithm', ('procedure',)),
    ('algorithmic-bias', ('concept',)),
    ('alpha-beta-pruning', ('search-algorithm',)),
    ('analogical-reasoning', ('thinking',)),
    ('animatronics', ('robotics',)),
    ('anomaly-detection', ('machine-learning',)),
    ('answer-set-programming', ('logic-programming',)),
    ('ant-colony-optimization', ('swarm-intelligence',)),
    ('apprenticeship-learning', ('imitation-learning',)),
    ('approximation-algorithm', ('algorithm',)),
    ('apriori-algorithm', ('association-rule-learning',)),
    ('argumentation-framework', ('concept',)),
    ('articulated-robot', ('automaton',)),
    ('artificial-chemistry', ('artificial-life',)),
    ('artificial-consciousness', ('field-of-study',)),
    ('artificial-empathy', ('artificial-intelligence',)),
    ('artificial-general-intelligence', ('artificial-intelligence',)),
    ('artificial-immune-system', ('computational-intelligence',)),
    ('artificial-intelligence', ('computer-science',)),
    ('artificial-life', ('field-of-study',)),
    ('artificial-neural-network', ('ai-model',)),
    ('artificial-society', ('agent-based-model',)),
    ('association-rule-learning', ('data-mining',)),
    ('attention-mechanism', ('concept',)),
    ('autoencoder', ('artificial-neural-network',)),
    ('automated-machine-learning', ('machine-learning',)),
    ('automated-planning', ('artificial-intelligence',)),
    ('automated-reasoning', ('artificial-intelligence',)),
    ('automated-theorem-proving', ('artificial-intelligence',)),
    ('automatic-summarization', ('text-mining',)),
    ('autonomic-computing', ('artificial-intelligence',)),
    ('autonomous-aircraft', ('autonomous-vehicle',)),
    ('autonomous-bus', ('autonomous-vehicle',)),
    ('autonomous-car', ('autonomous-vehicle',)),
    ('autonomous-robot', ('mobile-robot',)),
    ('autonomous-spacecraft', ('autonomous-vehicle',)),
    ('autonomous-truck', ('autonomous-vehicle',)),
    ('autonomous-underwater-vehicle', ('autonomous-vehicle',)),
    ('autonomous-vehicle', ('automaton', 'vehicle')),
    ('autonomous-weapon', ('military-robot',)),
    ('autoregressive-model', ('statistical-model',)),
    ('backpropagation', ('algorithm',)),
    ('backpropagation-through-time', ('backpropagation',)),
    ('backtracking-search', ('search-algorithm',)),
    ('backward-chaining', ('algorithm',)),
    ('bag-of-words-model', ('ai-model',)),
    ('barabasi-albert-model', ('statistical-model',)),
    ('batch-normalization', ('procedure',)),
    ('bayes-classifier', ('classification-algorithm',)),
    ('bayesian-network', ('statistical-model',)),
    ('bayesian-optimization', ('hyperparameter-optimization',)),
    ('behavior-based-robotics', ('robotics',)),
    ('behavior-tree', ('concept',)),
    ('belief-propagation', ('algorithm',)),
    ('belief-revision', ('concept',)),
    ('bellman-equation', ('concept',)),
    ('benchmark-dataset', ('dataset',)),
    ('bert-model', ('transformer-model',)),
    ('best-first-search', ('informed-search-algorithm',)),
    ('betweenness-centrality', ('centrality',)),
    ('bias-variance-tradeoff', ('concept',)),
    ('biclustering', ('cluster-analysis',)),
    ('bidirectional-associative-memory', ('recurrent-neural-network',)),
    ('bidirectional-recurrent-neural-network', ('recurrent-neural-network',)),
    ('bidirectional-search', ('search-algorithm',)),
    ('big-data', ('information',)),
    ('binary-classification', ('statistical-classification',)),
    ('bio-inspired-robotics', ('robotics',)),
    ('biorobotics', ('robotics',)),
    ('blackboard-system', ('software',)),
    ('blind-signal-separation', ('signal-processing',)),
    ('blocks-world', ('toy-problem',)),
    ('boltzmann-machine', ('energy-based-model',)),
    ('boolean-satisfiability-problem', ('constraint-satisfaction-problem',)),
    ('boosting-algorithm', ('ensemble-learning',)),
    ('bootstrap-aggregating', ('ensemble-learning',)),
    ('bounded-rationality', ('concept', 'decision-theory')),
    ('brain-computer-interface', ('device',)),
    ('breadth-first-search', ('graph-traversal',)),
    ('capsule-neural-network', ('convolutional-neural-network',)),
    ('case-based-reasoning', ('artificial-intelligence',)),
    ('catastrophic-forgetting', ('problem',)),
    ('causal-diagram', ('statistical-model',)),
    ('cellular-automaton', ('concept',)),
    ('centrality', ('concept',)),
    ('chatbot', ('software', 'software-agent')),
    ('chatbot-evaluation', ('procedure',)),
    ('chinese-room', ('concept',)),
    ('classification-algorithm', ('machine-learning-algorithm',)),
    ('closed-loop', ('control-system',)),
    ('closed-world-assumption', ('concept',)),
    ('cluster-analysis', ('data-mining', 'machine-learning', 'statistics')),
    ('clustering-algorithm', ('machine-learning-algorithm',)),
    ('cognitive-architecture', ('concept',)),
    ('cognitive-computing', ('artificial-intelligence',)),
    ('cognitive-robotics', ('robotics',)),
    ('cognitive-science', ('science',)),
    ('collaborative-filtering', ('machine-learning-algorithm',)),
    ('collaborative-robot', ('automaton',)),
    ('collective-intelligence', ('concept',)),
    ('commonsense-knowledge-base', ('knowledge-base',)),
    ('commonsense-reasoning', ('artificial-intelligence',)),
    ('community-detection', ('network-science',)),
    ('companion-robot', ('automaton',)),
    ('complex-adaptive-system', ('concept',)),
    ('complex-network', ('concept',)),
    ('compositional-pattern-producing-network', ('artificial-neural-network',)),
    ('compressed-sensing', ('signal-processing',)),
    ('computational-humor', ('computational-linguistics',)),
    ('computational-intelligence', ('artificial-intelligence',)),
    ('computational-learning-theory', ('machine-learning',)),
    ('computational-linguistics', ('linguistics',)),
    ('computational-neuroscience', ('science',)),
    ('computational-ontology', ('knowledge-base',)),
    ('computational-theory-of-mind', ('concept',)),
    ('computer-chess', ('artificial-intelligence',)),
    ('computer-go', ('artificial-intelligence',)),
    ('computer-simulation', ('method',)),
    ('computer-vision', ('machine-perception',)),
    ('concept-drift', ('concept',)),
    ('concurrent-logic-programming', ('logic-programming',)),
    ('conditional-random-field', ('statistical-model',)),
    ('confusion-matrix', ('model-evaluation-method',)),
    ('connectionism', ('concept',)),
    ('connectionist-expert-system', ('artificial-neural-network', 'expert-system')),
    ('constraint-logic-programming', ('constraint-programming', 'logic-programming')),
    ('constraint-programming', ('computer-science',)),
    ('constraint-satisfaction-problem', ('problem',)),
    ('content-addressable-memory', ('device',)),
    ('continual-learning', ('machine-learning',)),
    ('contrastive-learning', ('self-supervised-learning',)),
    ('control-system', ('artifact',)),
    ('control-theory', ('engineering', 'mathematics')),
    ('controlled-vocabulary', ('dataset',)),
    ('conversational-ai', ('natural-language-processing',)),
    ('convex-optimization', ('mathematical-optimization',)),
    ('convolutional-neural-network', ('feedforward-neural-network',)),
    ('cosine-similarity', ('similarity-measure',)),
    ('cross-validation', ('model-evaluation-method',)),
    ('cultural-algorithm', ('evolutionary-algorithm',)),
    ('curriculum-learning', ('machine-learning',)),
    ('cybernetics', ('information-science',)),
    ('data-augmentation', ('procedure',)),
    ('data-mining', ('computer-science', 'process')),
    ('data-poisoning', ('adversarial-machine-learning',)),
    ('dbscan', ('clustering-algorithm',)),
    ('decision-support-system', ('software',)),
    ('decision-theory', ('field-of-study',)),
    ('decision-tree', ('concept',)),
    ('decision-tree-learning', ('machine-learning-algorithm',)),
    ('deep-belief-network', ('artificial-neural-network',)),
    ('deep-generative-model', ('generative-model',)),
    ('deep-learning', ('machine-learning',)),
    ('deep-neural-network', ('artificial-neural-network',)),
    ('deep-q-network', ('artificial-neural-network', 'deep-reinforcement-learning')),
    ('deep-reinforcement-learning', ('deep-learning', 'reinforcement-learning')),
    ('deepfake', ('synthetic-media',)),
    ('default-logic', ('concept',)),
    ('delta-robot', ('automaton',)),
    ('dempster-shafer-theory', ('concept',)),
    ('denoising-autoencoder', ('autoencoder',)),
    ('depth-first-search', ('graph-traversal',)),
    ('developmental-robotics', ('robotics',)),
    ('diagnostic-expert-system', ('expert-system',)),
    ('dialogue-system', ('natural-language-processing',)),
    ('differential-evolution', ('evolutionary-algorithm',)),
    ('differential-privacy', ('concept',)),
    ('diffusion-model', ('generative-model', 'latent-variable-model')),
    ('digital-twin', ('computer-simulation',)),
    ('dijkstra-algorithm', ('search-algorithm',)),
    ('dimensionality-reduction', ('machine-learning',)),
    ('distant-supervision', ('weakly-supervised-learning',)),
    ('distilled-ai-model', ('ai-model',)),
    ('distributed-artificial-intelligence', ('artificial-intelligence',)),
    ('distributed-cognition', ('concept',)),
    ('distributional-semantics', ('computational-linguistics',)),
    ('document-classification', ('statistical-classification',)),
    ('domain-adaptation', ('transfer-learning',)),
    ('domestic-robot', ('service-robot',)),
    ('dynamic-bayesian-network', ('bayesian-network',)),
    ('dynamic-knowledge-graph', ('knowledge-graph',)),
    ('early-stopping', ('procedure',)),
    ('echo-state-network', ('recurrent-neural-network',)),
    ('edge-detection', ('image-processing',)),
    ('edge-of-chaos', ('concept',)),
    ('educational-data-mining', ('data-mining',)),
    ('eigenvector-centrality', ('centrality',)),
    ('elementary-cellular-automaton', ('cellular-automaton',)),
    ('elman-network', ('artificial-neural-network',)),
    ('embedding-model', ('ai-model',)),
    ('embodied-agent', ('intelligent-agent',)),
    ('embodied-artificial-intelligence', ('artificial-intelligence',)),
    ('embodied-cognition', ('concept',)),
    ('emotion-recognition', ('affective-computing',)),
    ('enactivism', ('concept',)),
    ('energy-based-model', ('ai-model', 'statistical-model')),
    ('ensemble-averaging', ('ensemble-learning',)),
    ('ensemble-learning', ('machine-learning',)),
    ('entertainment-robot', ('automaton',)),
    ('erdos-renyi-model', ('statistical-model',)),
    ('estimation-of-distribution-algorithm', ('evolutionary-algorithm',)),
    ('ethics-of-artificial-intelligence', ('philosophy-of-artificial-intelligence',)),
    ('evolutionary-algorithm', ('evolutionary-computation', 'metaheuristic')),
    ('evolutionary-computation', ('artificial-intelligence',)),
    ('evolutionary-robotics', ('robotics',)),
    ('example-based-machine-translation', ('machine-translation',)),
    ('existential-risk-from-agi', ('concept',)),
    ('expectation-maximization', ('algorithm',)),
    ('expectation-propagation', ('algorithm',)),
    ('expert-system', ('artificial-intelligence', 'knowledge-based-system')),
    ('explainability', ('attribute',)),
    ('explainable-ai', ('artificial-intelligence',)),
    ('explanation-based-learning', ('machine-learning',)),
    ('exploration-exploitation-dilemma', ('problem',)),
    ('extended-mind-thesis', ('concept',)),
    ('f1-score', ('model-evaluation-method',)),
    ('facial-recognition-system', ('software',)),
    ('factor-graph', ('concept',)),
    ('feature-detection', ('computer-vision',)),
    ('feature-extraction', ('dimensionality-reduction',)),
    ('feature-learning', ('machine-learning',)),
    ('feature-selection', ('dimensionality-reduction',)),
    ('federated-learning', ('machine-learning',)),
    ('feedback-loop', ('process',)),
    ('feedforward-neural-network', ('artificial-neural-network',)),
    ('few-shot-learning', ('machine-learning',)),
    ('fitness-function', ('concept',)),
    ('flow-based-generative-model', ('generative-model',)),
    ('forward-chaining', ('algorithm',)),
    ('forward-kinematics', ('robotics',)),
    ('foundation-model', ('ai-model',)),
    ('frame-based-expert-system', ('expert-system',)),
    ('frame-problem', ('problem',)),
    ('friendly-artificial-intelligence', ('artificial-general-intelligence',)),
    ('functional-logic-programming', ('logic-programming',)),
    ('fuzzy-set', ('concept',)),
    ('game-of-life', ('cellular-automaton',)),
    ('game-theory', ('concept',)),
    ('game-tree', ('concept',)),
    ('gated-recurrent-unit', ('recurrent-neural-network',)),
    ('gaussian-mixture-model', ('mixture-model',)),
    ('gaussian-process', ('concept',)),
    ('general-game-playing', ('artificial-intelligence',)),
    ('generalization-error', ('concept',)),
    ('generative-adversarial-network', ('generative-ai',)),
    ('generative-ai', ('artificial-intelligence', 'generative-model')),
    ('generative-ai-chatbot', ('chatbot',)),
    ('generative-model', ('ai-model', 'statistical-model')),
    ('generative-pre-trained-transformer', ('large-language-model',)),
    ('genetic-algorithm', ('evolutionary-algorithm', 'informed-search-algorithm')),
    ('genetic-programming', ('evolutionary-computation',)),
    ('geometric-feature-learning', ('feature-learning',)),
    ('gibbs-sampling', ('markov-chain-monte-carlo',)),
    ('gofai', ('symbolic-ai',)),
    ('goodharts-law', ('concept',)),
    ('governance-of-artificial-intelligence', ('management',)),
    ('gradient-boosting', ('boosting-algorithm',)),
    ('gradient-descent', ('algorithm',)),
    ('graph-attention-network', ('artificial-neural-network',)),
    ('graph-convolutional-network', ('artificial-neural-network',)),
    ('graph-embedding', ('vector-embedding',)),
    ('graph-neural-network', ('artificial-neural-network',)),
    ('graph-traversal', ('search-algorithm',)),
    ('greedy-algorithm', ('algorithm',)),
    ('grid-search', ('hyperparameter-optimization',)),
    ('handwriting-recognition', ('pattern-recognition',)),
    ('hebbian-learning', ('concept',)),
    ('heuristic', ('rule',)),
    ('heuristic-search-algorithm', ('search-algorithm',)),
    ('hidden-layer', ('neural-network-layer',)),
    ('hidden-markov-model', ('latent-variable-model',)),
    ('hierarchical-classifier', ('classification-algorithm',)),
    ('hierarchical-clustering', ('cluster-analysis',)),
    ('hierarchical-task-network', ('automated-planning',)),
    ('hierarchical-temporal-memory', ('artificial-neural-network',)),
    ('hill-climbing-search', ('search-algorithm',)),
    ('hits-algorithm', ('algorithm',)),
    ('hopfield-network', ('recurrent-neural-network',)),
    ('horn-clause', ('concept',)),
    ('human-based-computation', ('computer-science',)),
    ('human-computer-interaction', ('computer-science',)),
    ('human-robot-interaction', ('human-computer-interaction', 'robotics')),
    ('humanoid-robot', ('automaton',)),
    ('hyperparameter', ('concept',)),
    ('hyperparameter-optimization', ('automated-machine-learning', 'mathematical-optimization')),
    ('image-classification', ('computer-vision', 'image-processing')),
    ('image-dataset', ('dataset',)),
    ('image-processing', ('signal-processing',)),
    ('image-segmentation', ('computer-vision',)),
    ('image-to-image-translation', ('image-processing', 'machine-learning')),
    ('imitation-learning', ('robot-learning',)),
    ('incremental-learning', ('machine-learning',)),
    ('independent-component-analysis', ('unsupervised-learning',)),
    ('inductive-bias', ('concept',)),
    ('inductive-logic-programming', ('inductive-programming', 'logic-programming')),
    ('inductive-programming', ('machine-learning',)),
    ('industrial-robot', ('automaton',)),
    ('inference-engine', ('software',)),
    ('influence-diagram', ('concept',)),
    ('information-bottleneck', ('concept',)),
    ('information-extraction', ('information-retrieval', 'natural-language-processing')),
    ('information-retrieval', ('computer-science',)),
    ('information-science', ('science',)),
    ('informed-search-algorithm', ('search-algorithm',)),
    ('instance-based-learning', ('machine-learning',)),
    ('instance-segmentation', ('image-segmentation',)),
    ('instruction-tuned-model', ('large-language-model',)),
    ('instrumental-convergence', ('concept',)),
    ('intelligence-amplification', ('concept',)),
    ('intelligent-agent', ('agent',)),
    ('intelligent-system', ('software',)),
    ('intelligent-tutoring-system', ('software',)),
    ('interactive-machine-learning', ('machine-learning',)),
    ('internet-bot', ('software-agent',)),
    ('inverse-kinematics', ('robotics',)),
    ('inverse-reinforcement-learning', ('reinforcement-learning',)),
    ('isomap', ('nonlinear-dimensionality-reduction',)),
    ('iterative-deepening-depth-first-search', ('depth-first-search',)),
    ('junction-tree-algorithm', ('algorithm',)),
    ('k-means-clustering', ('clustering-algorithm',)),
    ('k-nearest-neighbors', ('classification-algorithm',)),
    ('kalman-filter', ('algorithm',)),
    ('kernel-method', ('machine-learning-algorithm',)),
    ('knowledge-base', ('dataset',)),
    ('knowledge-based-agent', ('intelligent-agent',)),
    ('knowledge-based-system', ('software',)),
    ('knowledge-distillation', ('machine-learning',)),
    ('knowledge-engineering', ('artificial-intelligence',)),
    ('knowledge-extraction', ('data-mining',)),
    ('knowledge-graph', ('knowledge-base',)),
    ('knowledge-graph-embedding', ('graph-embedding',)),
    ('knowledge-management', ('management',)),
    ('knowledge-representation', ('artificial-intelligence',)),
    ('kolmogorov-complexity', ('concept',)),
    ('langtons-ant', ('cellular-automaton',)),
    ('language-model', ('ai-model', 'statistical-model')),
    ('large-action-model', ('large-language-model',)),
    ('large-language-model', ('language-model',)),
    ('latent-dirichlet-allocation', ('generative-model',)),
    ('latent-space', ('concept',)),
    ('latent-variable-model', ('statistical-model',)),
    ('lazy-learning', ('machine-learning',)),
    ('learning-analytics', ('field-of-study',)),
    ('learning-rate', ('concept',)),
    ('lemmatization', ('natural-language-processing',)),
    ('lidar', ('device',)),
    ('life-like-cellular-automaton', ('cellular-automaton',)),
    ('linear-classifier', ('classification-algorithm',)),
    ('linear-discriminant-analysis', ('classification-algorithm',)),
    ('linear-programming', ('mathematical-optimization',)),
    ('link-prediction', ('network-science',)),
    ('linked-data', ('information',)),
    ('linked-open-data', ('dataset', 'linked-data')),
    ('locality-sensitive-hashing', ('algorithm',)),
    ('logic-programming', ('computer-science',)),
    ('logic-programming-language', ('language',)),
    ('logistic-regression', ('classification-algorithm', 'statistical-model')),
    ('long-short-term-memory', ('recurrent-neural-network',)),
    ('loss-function', ('concept',)),
    ('machine-ethics', ('ethics-of-artificial-intelligence',)),
    ('machine-learning', ('artificial-intelligence',)),
    ('machine-learning-algorithm', ('algorithm',)),
    ('machine-perception', ('artificial-intelligence',)),
    ('machine-reading-comprehension', ('natural-language-processing',)),
    ('machine-translation', ('natural-language-processing',)),
    ('markov-chain', ('concept',)),
    ('markov-chain-monte-carlo', ('monte-carlo-method',)),
    ('markov-decision-process', ('concept',)),
    ('masked-language-model', ('language-model',)),
    ('mathematical-optimization', ('mathematics',)),
    ('maximum-likelihood-estimation', ('procedure',)),
    ('medical-robot', ('automaton',)),
    ('membership-inference-attack', ('adversarial-machine-learning',)),
    ('memetic-algorithm', ('evolutionary-algorithm',)),
    ('memory-augmented-neural-network', ('artificial-neural-network',)),
    ('meta-learning', ('machine-learning',)),
    ('metaheuristic', ('algorithm',)),
    ('metropolis-hastings-algorithm', ('markov-chain-monte-carlo',)),
    ('microbot', ('automaton',)),
    ('microbotics', ('robotics',)),
    ('military-robot', ('automaton',)),
    ('minimax-rule', ('algorithm',)),
    ('minimum-description-length', ('concept',)),
    ('mixture-model', ('latent-variable-model',)),
    ('mixture-of-experts-model', ('large-language-model', 'transformer-model')),
    ('mobile-robot', ('automaton',)),
    ('mobile-robotics', ('robotics',)),
    ('model-evaluation-method', ('machine-learning',)),
    ('model-free-reinforcement-learning', ('reinforcement-learning',)),
    ('model-predictive-control', ('control-theory',)),
    ('modular-neural-network', ('artificial-neural-network',)),
    ('monte-carlo-algorithm', ('algorithm',)),
    ('monte-carlo-method', ('algorithm',)),
    ('monte-carlo-tree-search', ('search-algorithm',)),
    ('morphological-analysis', ('natural-language-processing',)),
    ('motion-planning', ('robotics',)),
    ('multi-agent-reinforcement-learning', ('reinforcement-learning',)),
    ('multi-agent-system', ('software',)),
    ('multi-armed-bandit', ('problem',)),
    ('multi-head-attention', ('attention-mechanism',)),
    ('multi-task-learning', ('machine-learning',)),
    ('multiclass-classification', ('statistical-classification',)),
    ('multilayer-perceptron', ('feedforward-neural-network',)),
    ('multimodal-artificial-intelligence', ('artificial-intelligence',)),
    ('multimodal-large-language-model', ('large-language-model',)),
    ('multimodal-learning', ('machine-learning',)),
    ('multiple-instance-learning', ('supervised-learning',)),
    ('mutual-information', ('concept',)),
    ('n-gram', ('concept',)),
    ('naive-bayes-classifier', ('bayes-classifier',)),
    ('named-entity-recognition', ('information-extraction',)),
    ('nanorobotics', ('robotics',)),
    ('natural-language-generation', ('natural-language-processing',)),
    ('natural-language-processing', ('artificial-intelligence', 'computational-linguistics', 'information-science')),
    ('natural-language-understanding', ('natural-language-processing',)),
    ('nearest-neighbor-classifier', ('classification-algorithm',)),
    ('neat-algorithm', ('neuroevolution',)),
    ('negative-feedback', ('feedback-loop',)),
    ('network-science', ('field-of-study',)),
    ('neural-architecture-search', ('automated-machine-learning',)),
    ('neural-machine-translation', ('machine-translation',)),
    ('neural-network-layer', ('concept',)),
    ('neural-network-model', ('artificial-neural-network',)),
    ('neural-radiance-field', ('ai-model',)),
    ('neural-turing-machine', ('memory-augmented-neural-network',)),
    ('neuro-symbolic-ai', ('artificial-intelligence',)),
    ('neuroevolution', ('evolutionary-computation',)),
    ('neuromorphic-engineering', ('engineering',)),
    ('neurorobotics', ('robotics',)),
    ('no-free-lunch-theorem', ('concept',)),
    ('non-negative-matrix-factorization', ('algorithm',)),
    ('nonlinear-dimensionality-reduction', ('dimensionality-reduction',)),
    ('nouvelle-ai', ('artificial-intelligence',)),
    ('object-detection', ('computer-vision',)),
    ('obstacle-avoidance', ('path-planning', 'robot-navigation')),
    ('one-class-classification', ('statistical-classification',)),
    ('one-shot-learning', ('few-shot-learning',)),
    ('online-algorithm', ('algorithm',)),
    ('online-machine-learning', ('incremental-learning',)),
    ('ontology-alignment', ('knowledge-engineering',)),
    ('ontology-engineering', ('knowledge-engineering',)),
    ('ontology-learning', ('knowledge-engineering',)),
    ('open-world-assumption', ('concept',)),
    ('operations-research', ('research',)),
    ('optical-character-recognition', ('pattern-recognition',)),
    ('optical-flow', ('concept',)),
    ('optimal-control', ('control-theory',)),
    ('overfitting', ('concept',)),
    ('pac-learning', ('computational-learning-theory', 'concept')),
    ('pagerank', ('algorithm',)),
    ('panoptic-segmentation', ('image-segmentation',)),
    ('parser', ('computer-program',)),
    ('part-of-speech-tagging', ('natural-language-processing',)),
    ('partial-order-planning', ('automated-planning',)),
    ('particle-filter', ('algorithm',)),
    ('particle-swarm-optimization', ('swarm-intelligence',)),
    ('path-planning', ('motion-planning',)),
    ('pathfinding', ('search-algorithm',)),
    ('pattern-recognition', ('machine-learning',)),
    ('pddl', ('language',)),
    ('pedestrian-detection', ('object-detection',)),
    ('perceptron', ('feedforward-neural-network', 'linear-classifier')),
    ('personal-robot', ('automaton',)),
    ('philosophy-of-artificial-intelligence', ('philosophy',)),
    ('policy-gradient-method', ('reinforcement-learning',)),
    ('positive-feedback', ('feedback-loop',)),
    ('precision-and-recall', ('model-evaluation-method',)),
    ('predictive-modelling', ('machine-learning',)),
    ('preference-elicitation', ('procedure',)),
    ('preference-learning', ('machine-learning',)),
    ('preferential-attachment', ('process',)),
    ('principal-component-analysis', ('dimensionality-reduction',)),
    ('prompt-engineering', ('procedure',)),
    ('proximal-policy-optimization', ('model-free-reinforcement-learning', 'policy-gradient-method')),
    ('pulse-coupled-network', ('artificial-neural-network',)),
    ('q-learning', ('model-free-reinforcement-learning',)),
    ('qualitative-reasoning', ('artificial-intelligence',)),
    ('quantum-machine-learning', ('machine-learning',)),
    ('quantum-neural-network', ('artificial-neural-network',)),
    ('question-answering', ('natural-language-processing',)),
    ('rademacher-complexity', ('concept',)),
    ('radial-basis-function', ('concept',)),
    ('radial-basis-function-network', ('artificial-neural-network',)),
    ('random-forest', ('ensemble-learning',)),
    ('rational-agent', ('agent',)),
    ('rdf-graph', ('dataset',)),
    ('reactive-planning', ('automated-planning',)),
    ('reasoning-language-model', ('large-language-model',)),
    ('receiver-operating-characteristic', ('model-evaluation-method',)),
    ('recommender-system', ('software',)),
    ('recurrent-neural-network', ('artificial-neural-network',)),
    ('recursive-neural-network', ('recurrent-neural-network',)),
    ('region-based-convolutional-neural-network', ('convolutional-neural-network',)),
    ('regulation-of-artificial-intelligence', ('law',)),
    ('reinforcement-learning', ('machine-learning',)),
    ('reinforcement-learning-from-human-feedback', ('reinforcement-learning',)),
    ('relationship-extraction', ('text-mining',)),
    ('rescue-robot', ('automaton',)),
    ('reservoir-computing', ('recurrent-neural-network',)),
    ('residual-neural-network', ('artificial-neural-network',)),
    ('resource-description-framework', ('concept',)),
    ('restricted-boltzmann-machine', ('boltzmann-machine', 'stochastic-neural-network')),
    ('retrieval-augmented-generation', ('generative-ai', 'natural-language-processing')),
    ('reversible-cellular-automaton', ('cellular-automaton',)),
    ('reward-hacking', ('specification-gaming',)),
    ('roboethics', ('ethics-of-artificial-intelligence',)),
    ('robot-control', ('robotics',)),
    ('robot-end-effector', ('device',)),
    ('robot-learning', ('machine-learning',)),
    ('robot-navigation', ('robotics',)),
    ('robot-planning', ('robotics',)),
    ('robot-rights', ('concept',)),
    ('robotic-arm', ('automaton',)),
    ('robotic-manipulator', ('machine',)),
    ('robotic-spacecraft', ('automaton',)),
    ('robotics', ('artificial-intelligence',)),
    ('robust-control', ('control-theory',)),
    ('rule-based-expert-system', ('expert-system',)),
    ('rule-based-machine-learning', ('machine-learning',)),
    ('rule-based-machine-translation', ('machine-translation',)),
    ('rule-based-system', ('knowledge-based-system',)),
    ('rule-induction', ('machine-learning',)),
    ('sample-complexity', ('concept',)),
    ('sat-solver', ('software',)),
    ('satisficing', ('concept',)),
    ('scalable-oversight', ('ai-alignment',)),
    ('scale-free-network', ('complex-network',)),
    ('search-algorithm', ('algorithm',)),
    ('self-attention', ('attention-mechanism',)),
    ('self-organization', ('process',)),
    ('self-organizing-map', ('artificial-neural-network',)),
    ('self-reconfiguring-modular-robot', ('automaton',)),
    ('self-supervised-learning', ('unsupervised-learning',)),
    ('semantic-analysis', ('natural-language-processing',)),
    ('semantic-network', ('knowledge-representation',)),
    ('semantic-parsing', ('natural-language-processing',)),
    ('semantic-reasoner', ('inference-engine',)),
    ('semantic-role-labeling', ('natural-language-processing',)),
    ('semantic-segmentation', ('image-segmentation',)),
    ('semantic-web', ('concept',)),
    ('semi-supervised-learning', ('machine-learning',)),
    ('sensor-fusion', ('machine-perception',)),
    ('sentence-embedding', ('feature-learning', 'natural-language-processing')),
    ('sentiment-analysis', ('affective-computing', 'natural-language-processing')),
    ('sequence-labeling', ('pattern-recognition',)),
    ('sequence-to-sequence-learning', ('machine-learning',)),
    ('service-robot', ('automaton',)),
    ('servo', ('control-system', 'mechanism')),
    ('sigmoid-function', ('activation-function',)),
    ('signal-processing', ('engineering',)),
    ('similarity-measure', ('concept',)),
    ('simulated-annealing', ('metaheuristic',)),
    ('simultaneous-localization-and-mapping', ('robot-navigation',)),
    ('situated-cognition', ('concept',)),
    ('situation-calculus', ('concept',)),
    ('small-language-model', ('language-model',)),
    ('small-world-network', ('complex-network',)),
    ('social-robot', ('automaton',)),
    ('social-robotics', ('robotics',)),
    ('soft-robotics', ('robotics',)),
    ('softmax-function', ('activation-function',)),
    ('sorting-algorithm', ('algorithm',)),
    ('sparql', ('language',)),
    ('sparse-dictionary-learning', ('feature-learning',)),
    ('sparse-distributed-memory', ('ai-model',)),
    ('spatial-temporal-reasoning', ('artificial-intelligence',)),
    ('speaker-verification', ('signal-processing',)),
    ('specification-gaming', ('problem',)),
    ('speech-dataset', ('dataset',)),
    ('speech-recognition', ('machine-perception', 'natural-language-processing')),
    ('speech-synthesis', ('natural-language-processing',)),
    ('spiking-neural-network', ('artificial-neural-network',)),
    ('state-observer', ('concept',)),
    ('state-space-search', ('search-algorithm',)),
    ('statistical-classification', ('machine-learning',)),
    ('statistical-language-model', ('language-model',)),
    ('statistical-learning-theory', ('computational-learning-theory', 'concept')),
    ('statistical-machine-translation', ('machine-translation',)),
    ('statistical-model', ('concept',)),
    ('statistical-relational-learning', ('machine-learning',)),
    ('stemmer', ('algorithm',)),
    ('stemming', ('natural-language-processing',)),
    ('stochastic-block-model', ('generative-model',)),
    ('stochastic-gradient-descent', ('gradient-descent',)),
    ('stochastic-neural-network', ('artificial-neural-network',)),
    ('stop-word', ('concept',)),
    ('string-searching-algorithm', ('search-algorithm',)),
    ('structure-from-motion', ('3d-reconstruction',)),
    ('structured-prediction', ('machine-learning',)),
    ('subsumption-architecture', ('cognitive-architecture',)),
    ('superintelligence', ('artificial-general-intelligence',)),
    ('supervised-learning', ('machine-learning',)),
    ('support-vector-machine', ('classification-algorithm', 'supervised-learning')),
    ('swarm-intelligence', ('artificial-intelligence',)),
    ('swarm-robotics', ('robotics',)),
    ('symbol-grounding-problem', ('problem',)),
    ('symbolic-ai', ('artificial-intelligence',)),
    ('syntactic-parsing', ('natural-language-processing',)),
    ('synthetic-data', ('information',)),
    ('synthetic-media', ('information',)),
    ('system-identification', ('control-theory',)),
    ('t-sne', ('nonlinear-dimensionality-reduction',)),
    ('tabu-search', ('metaheuristic',)),
    ('teacher-forcing', ('procedure',)),
    ('teleoperation', ('procedure',)),
    ('telerobotics', ('robotics',)),
    ('temporal-knowledge-graph', ('knowledge-graph',)),
    ('text-classification', ('natural-language-processing', 'statistical-classification')),
    ('text-mining', ('natural-language-processing',)),
    ('text-segmentation', ('natural-language-processing',)),
    ('text-to-image-model', ('generative-model',)),
    ('text-to-video-model', ('generative-model',)),
    ('text-tokenization', ('natural-language-processing',)),
    ('tf-idf', ('concept',)),
    ('thompson-sampling', ('algorithm',)),
    ('topic-model', ('latent-variable-model',)),
    ('toy-problem', ('problem',)),
    ('training-set', ('dataset',)),
    ('transductive-learning', ('supervised-learning',)),
    ('transfer-learning', ('machine-learning',)),
    ('transformer-model', ('artificial-neural-network',)),
    ('tree-traversal', ('graph-traversal',)),
    ('triplestore', ('database', 'software')),
    ('turing-test', ('procedure',)),
    ('u-net', ('convolutional-neural-network',)),
    ('uncanny-valley', ('concept',)),
    ('underwater-robot', ('automaton',)),
    ('uniform-cost-search', ('search-algorithm',)),
    ('uninformed-search-algorithm', ('search-algorithm',)),
    ('universal-approximation-theorem', ('concept',)),
    ('unmanned-aerial-vehicle', ('aircraft',)),
    ('unsupervised-learning', ('machine-learning',)),
    ('upper-confidence-bound', ('algorithm',)),
    ('user-modeling', ('concept',)),
    ('value-alignment', ('ai-alignment',)),
    ('value-function', ('concept',)),
    ('vanishing-gradient-problem', ('problem',)),
    ('variational-autoencoder', ('autoencoder',)),
    ('vector-database', ('software',)),
    ('vector-embedding', ('concept',)),
    ('virtual-assistant', ('software-agent',)),
    ('virtual-human', ('computer-simulation',)),
    ('vision-transformer', ('transformer-model',)),
    ('visual-odometry', ('computer-vision',)),
    ('voice-user-interface', ('software',)),
    ('weak-artificial-intelligence', ('artificial-intelligence',)),
    ('weakly-supervised-learning', ('supervised-learning',)),
    ('web-crawler', ('information', 'internet-bot')),
    ('web-ontology-language', ('language',)),
    ('weighted-constraint-satisfaction-problem', ('constraint-satisfaction-problem',)),
    ('winner-take-all-network', ('recurrent-neural-network',)),
    ('word-embedding', ('vector-embedding',)),
    ('word-sense-disambiguation', ('natural-language-processing',)),
    ('world-model', ('ai-model',)),
    ('zero-shot-learning', ('machine-learning',)),
    ('database', ()),
)
