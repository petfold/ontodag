...
currently state separately — holdings' projection contract (its
...
1. **Sources of truth.** Authoritative, original data, each with an
   ...
   (observed by holdings), message logs and their envelopes (ucomm),
...
2. **Projections.** Machine-derived views of a source, held in the DAG
   under a reserved namespace (`sys:` in holdings' contract). A
   ...
   `sys:backup:<n>` (holdings); author/channel/time/media facets of
...
## 3. The projection rules (agreed — holdings' contract, generalized)
...
The wire format is holdings', adopted as the family standard: JSON
lines, one item per line —

    {"item": "<name>", "supercategories": ["sys:...", "sys:..."]}

— which is `put(item, supercategories)` verbatim. Any source-side
projector (holdings' `project-ontodag`, ucomm's future envelope
...
holdings' `ontodag_ingest.py` is a pre-API template of exactly this;
...
- **Files: content hash** (`sha256:…`), paths and media as attributes —
  holdings' decision, adopted. Dedup is free; a copy is one item.
...
- **Facts and norms stay in different layers.** Projections report
  ...
  The enforcement report is their join: holdings' `redundancy` /
...
prelude-tier, conflict-detectable. holdings' flat `sys:type:<ext>` is
...
| repo | owns | owed |
|---|---|---|
| **holdings** | filesystem + backup scanning, SQLite source of facts, `project-ontodag` (exists) | retention-aware redundancy join (§7); v0.3 "OntoDAG join live" consumes §5's overlay view |
...
1. **Rebuild cost at scale.** Idempotent full rebuild of 10⁵–10⁶
   ...
   **MEASURED (2026-08-20, `experiments/projection_scale.py`; synthetic
   holdings-shaped streams, plus a real run over this machine's project
