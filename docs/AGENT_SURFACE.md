...
| `query` | `terms` (conjunction) **or** `any_of` (list of conjunctions, answered as their union) — at most one, and neither means the empty query; `items_only?` (leaves that are not typed values); `limit?`, `as_of?`, `certify?` | `items` (sorted canonical names), `count`, `truncated`, and the **canonical echo** of the terms actually answered |
