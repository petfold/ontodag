...
- **The ten domain packs ship in the wheel**: `odag pack physics`,
  `mathematics`, `chemistry`, `biology`, `medicine`, `ai`, `economics`,
  `computing`, `geography`, `space` — 6,793 categories over core, built by
  consensus over WordNet and Wikidata with hand rulings in ontodag-core
  under core's policy (hinges in core, contents in packs, no everyday word
  taken by a pack; economics without macroeconomics, geography without
  countries, space without numbers). Each applies `core` first
  (`packs.presumes_core`), as core applies the prelude. A name a pack hangs
  things from that belongs to a *sibling* pack (geography's GIS formats
  under computing's `file-format`) is entered parentless, so the pack
  adopts alone — the borrowed name sits at top level until the sibling is
  adopted and is filed by that merge; `odag pack` counts and the
  missing-parent hint exclude such names. Golden roots pinned per pack under
  both addressings, plus the **union**: core and all ten, in any order,
  9,793 categories, root `759ebb5a…` — reproduced from the shipped modules
  and by ontodag-core's `tools/integrate.py` from the pack files. Modules in
  `ontodag.domain`, generated by ontodag-core's
  `tools/regen_ontodag_packs.py`. Shipping in the wheel is the reversible
  half of PACKS.md's distribution question: publishing them to Swarm later
  is the same stores.

### Documentation

- Guide §4.8 gains the domain packs (executed snippets, including the
  borrowed-name case); REFERENCE.md lists them; CORE.md's v4 section says
  they ship.
